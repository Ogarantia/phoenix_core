...
template <typename T>
void decomposeQuaternionInputs(const TensorSplit<device::CUDA, const T, 4>& inLeft, TemporaryTensor<device::CUDA, T>* outLeft,
...
template <typename T>
void decomposeQuaternionOutputGrad(const TensorSplit<device::CUDA, const T, 4>& inGrad, TemporaryTensor<device::CUDA, T>* outGrad) {
...
template <typename T>
void recomposeQuaternionOutput(TemporaryTensor<device::CUDA, T>* inLanes, TensorSplit<device::CUDA, T, 4>& outQuats) {
...
template <typename T>
void recomposeQuaternionInputsGrad(TemporaryTensor<device::CUDA, T>* inLeftGradLanes, TensorSplit<device::CUDA, T, 4>& outLeftGradQuats,
...
template <>
void decomposeQuaternionInputs(const TensorSplit<device::CUDA, const float, 4>& inLeft, TemporaryTensor<device::CUDA, float>* outLeft,
                               const TensorSplit<device::CUDA, const float, 4>& inRight, TemporaryTensor<device::CUDA, float>* outRight) {
    ::decomposeQuaternionInputs(inLeft, outLeft, inRight, outRight);
}

template <>
void decomposeQuaternionOutputGrad(const TensorSplit<device::CUDA, const float, 4>& inGrad, TemporaryTensor<device::CUDA, float>* outGrad) {
    ::decomposeQuaternionOutputGrad(inGrad, outGrad);
}

template <>
void recomposeQuaternionOutput(TemporaryTensor<device::CUDA, float>* inLanes, TensorSplit<device::CUDA, float, 4>& outQuats) {
    ::recomposeQuaternionOutput(inLanes, outQuats);
}

template <>
void recomposeQuaternionInputsGrad(TemporaryTensor<device::CUDA, float>* inLeftGradLanes, TensorSplit<device::CUDA, float, 4>& outLeftGradQuats,
                                   TemporaryTensor<device::CUDA, float>* inRightGradLanes, TensorSplit<device::CUDA, float, 4>& outRightGradQuats) {
...
template <>
void decomposeQuaternionInputs(const TensorSplit<device::CUDA, const half, 4>& inLeft, TemporaryTensor<device::CUDA, half>* outLeft,
                               const TensorSplit<device::CUDA, const half, 4>& inRight, TemporaryTensor<device::CUDA, half>* outRight) {
    ::decomposeQuaternionInputs(inLeft, outLeft, inRight, outRight);
    cudnn::Context::raiseIfError();
}

template <>
void decomposeQuaternionOutputGrad(const TensorSplit<device::CUDA, const half, 4>& inGrad, TemporaryTensor<device::CUDA, half>* outGrad) {
    ::decomposeQuaternionOutputGrad(inGrad, outGrad);
    cudnn::Context::raiseIfError();
}

template <>
void recomposeQuaternionOutput(TemporaryTensor<device::CUDA, half>* inLanes, TensorSplit<device::CUDA, half, 4>& outQuats) {
    ::recomposeQuaternionOutput(inLanes, outQuats);
    cudnn::Context::raiseIfError();
}

template <>
void recomposeQuaternionInputsGrad(TemporaryTensor<device::CUDA, half>* inLeftGradLanes, TensorSplit<device::CUDA, half, 4>& outLeftGradQuats,
                                   TemporaryTensor<device::CUDA, half>* inRightGradLanes, TensorSplit<device::CUDA, half, 4>& outRightGradQuats) {
    ::recomposeQuaternionInputsGrad(inLeftGradLanes, outLeftGradQuats, inRightGradLanes, outRightGradQuats);
    cudnn::Context::raiseIfError();
}
#endif
